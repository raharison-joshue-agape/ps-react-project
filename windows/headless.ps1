$react_vite_headless_project_config_content = @'
...
'@
$react_vite_headless_project_tsconfig_app_content = @'
{
    "compilerOptions": {
...
'@
$react_vite_headless_project_main_content = @'
...
'@
$react_vite_headless_project_theme_provider_content = @'
...
'@
$react_vite_headless_project_style_content = @'
@import 'tailwindcss';

@custom-variant dark (&:is(.dark *));
'@
$react_vite_headless_project_route_content = @'
...
'@
$react_vite_headless_project_home_page_content = @'
    ...
    return (
        <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 py-16 text-center bg-linear-to-br from-slate-50 via-white to-indigo-50 transition-colors dark:from-gray-950 dark:via-gray-900 dark:to-indigo-950">
            ...
            <p className="mb-10 max-w-2xl text-lg leading-relaxed text-gray-600 sm:text-xl dark:text-gray-300">
                ...
                <span className="font-semibold text-cyan-600 dark:text-cyan-400">Headless UI</span>,
...
'@
$react_vite_headless_project_not_found_page_content = @'
import ToggleMode from '@/components/ToggleMode';
...
'@
$react_vite_headless_project_default_layout_content = @'
...
'@
$react_vite_headless_project_toggle_mode_content = @'
...
export default function ToggleMode({ className }: { className?: string }) {
    ...
    return (
        <div className={className}>
            <Menu>
                <MenuButton className="inline-flex h-9 items-center gap-2 rounded-full border border-gray-300 bg-white/80 px-4 text-sm font-medium text-gray-700 shadow-sm backdrop-blur transition-colors hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-900/80 dark:text-gray-200 dark:hover:bg-gray-800">
                    <CurrentIcon size={16} />
                    {theme}
                    <ChevronDown size={14} />
                </MenuButton>
                ...
                <MenuItems
                    anchor="bottom end"
                    className="z-50 mt-2 w-40 rounded-xl border border-gray-200 bg-white p-1.5 shadow-lg dark:border-gray-700 dark:bg-gray-900"
                >
                    {OPTIONS.map(({ value, label, Icon }) => (
                        <MenuItem key={value}>
                            <button
                                ...
                                className="flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-700 outline-none data-[focus]:bg-gray-100 dark:text-gray-200 dark:data-[focus]:bg-gray-800"
                            >
                                <Icon size={16} />
                                {label}
                            </button>
...
<#
.SYNOPSIS
    Crée un projet Vite + React 19 + Headless UI + TypeScript pré-configuré.

.DESCRIPTION
    Installe Headless UI (Tailwind Labs), TailwindCSS et Lucide Icons, puis génère le layout, les pages Home/404 et le sélecteur de thème (Clair / Sombre / Système via un menu Headless UI et la classe .dark).
...
#>
function New-ReactViteHeadless {
    param([string]$PROJECT_NAME)

    if (-not $PROJECT_NAME) { $PROJECT_NAME = Read-Host "Project name" }

    Write-Host "Creating project: $PROJECT_NAME (Vite + React 19 + Headless UI + TypeScript)"
    ...
    npm install react-router-dom tailwindcss @tailwindcss/vite @headlessui/react lucide-react
    ...
    Set-Content "vite.config.ts" -Value $react_vite_headless_project_config_content -Encoding UTF8
    Set-Content "tsconfig.app.json" -Value $react_vite_headless_project_tsconfig_app_content -Encoding UTF8
    Set-Content "src/main.tsx" -Value $react_vite_headless_project_main_content -Encoding UTF8
    Set-Content "src/theme/ThemeProvider.tsx" -Value $react_vite_headless_project_theme_provider_content -Encoding UTF8
    Set-Content "src/styles/index.css" -Value $react_vite_headless_project_style_content -Encoding UTF8
    Set-Content "src/routes/index.tsx" -Value $react_vite_headless_project_route_content -Encoding UTF8
    Set-Content "src/pages/Home.tsx" -Value $react_vite_headless_project_home_page_content -Encoding UTF8
    Set-Content "src/pages/NotFound.tsx" -Value $react_vite_headless_project_not_found_page_content -Encoding UTF8
    Set-Content "src/layouts/default.tsx" -Value $react_vite_headless_project_default_layout_content -Encoding UTF8
    Set-Content "src/components/ToggleMode.tsx" -Value $react_vite_headless_project_toggle_mode_content -Encoding UTF8
